...
class Bank:
    ...
    @property
    def user_list(self) : return self.__user_list

    @property 
    def atm_list(self) : return self.__atm_list

    @property 
    def edc_list(self) : return self.__edc_list
...
class Account:
    ...
    def deposit(self, bank: Bank, channel_no, amount):
        if 'ATM' in channel_no:
            for atm in bank.atm_list:
                if atm.atm_no == channel_no:
                    self.__balance += amount
                    transaction = Transaction("D-ATM", channel_no.replace('ATM' , ''), amount, int(self.__balance))
                    self.__transaction.append(transaction)
                    return 'Success'
        elif 'EDC' in channel_no:           
            pass  
        elif 'COUNTER' in channel_no:          
            self.__balance += amount
            transaction = Transaction("D-COUNTER", channel_no.replace('COUNTER' , ''), amount, int(self.__balance))
            self.__transaction.append(transaction)
            return 'Success'
        return 'Error'
    
    def withdraw(self , bank : Bank , channel_no , amount):
        if 'ATM' in channel_no:
            for atm in bank.atm_list:
                if atm.branch_no == channel_no:
                    self.__balance -= amount
                    transaction = Transaction("W-ATM", channel_no.replace('ATM' , ''), amount, int(self.__balance))
                    self.__transaction.append(transaction)
                    return 'Success'
        elif 'EDC' in channel_no:           
            pass  
        elif 'COUNTER' in channel_no:          
            if amount > self.__balance : return "Error : not enough money"
            self.__balance -= amount
            transaction = Transaction("W-COUNTER", channel_no.replace('COUNTER' , ''), amount, int(self.__balance))
            self.__transaction.append(transaction)
            return 'Success'
        elif 'SYSTEM' in channel_no :
            self.__balance -= amount
            transaction = Transaction("W-SYSTEM", channel_no.replace('SYSTEM' , ''), amount, int(self.__balance))
            self.__transaction.append(transaction)
            return 'Success'
        return 'Error'
    ...
    def add_transaction(self , sth) :
        self.__transaction.append(sth)
...
class ATMMachine(TransactionChannel):
    ...
    def deposit(self, account, amount):
        ...
        if account.deposit(self.__bank, self.__atm_no, amount) :
            self.__money += amount
            return 'Success'
        return 'Error'
    ...
    def withdraw(self, account, amount):
        if account.withdraw( self.__bank ,self.__atm_no, amount):
             self.__money -= amount
             return 'Success'
        return 'Error'
    ...
    def transfer(self, account, target_account, amount):
        return account.transfer(self.__atm_no, amount, target_account)
...
class Counter(TransactionChannel):
    ...
    def deposit(self, account, amount, account_id, citizen_id):
        if self.verify_identity(account, account_id, citizen_id):
            if account.deposit(self.__bank ,self.__branch_no, amount) :
                return 'Success'
            return 'Error'
    ...
    def withdraw(self, account, amount, account_id, citizen_id):
        if self.verify_identity(account, account_id, citizen_id):
            return account.withdraw( self.bank ,self.__branch_no, amount)
        return "Error: Invalid identity"
    
    def transfer(self, account, target_account, amount, account_id, citizen_id):
        if self.verify_identity(account, account_id, citizen_id):
            return account.transfer(self.__branch_no, amount, target_account)
        return "Error: Invalid identity"
...
class EDCMachine(TransactionChannel):
    ...
    def calculate_cashback(self , bank : Bank, debit_card ,amount):
        cashback = amount * debit_card.credit 
        return cashback
        
    def pay(self , bank : Bank, debit_card ,amount):
        if self.__current_card != debit_card: return 'Error: No card inserted'
        card = bank.search_account(debit_card.account_no)
        if not hasattr(debit_card , 'pay_minumum' ):
            return card.pay(bank , debit_card , amount)
        else :
            if  amount  >= debit_card.pay_minimum:
                cashback = self.calculate_cashback(bank , debit_card , amount)
                return card.pay(bank , debit_card , amount)
...
class SavingAccount(Account) :
    ...
    def __init__(self, account_no, user, balance):
        ...
        self.__max_withdraw = 50000
        self.__balance = balance

    def withdraw(self, bank, channel_no, amount):
        if amount > self.__max_withdraw : return False
        return super().withdraw(bank, channel_no, amount)
    
    def calculate_interest(self , year) :
        interest = math.ceil(self.__balance * pow((1 + self.__interest), int(year)) - self.__balance)
        self.add_balance(interest)
        self.add_transaction(Transaction("I-", 0 , interest , int(self.__balance)))
        return  interest        

    def pay(self, bank : Bank , debit_card ,amount) :
        if amount < 0  : return 'Error'
        if amount > self.__balance : return 'Error'
        self.remove_balance(amount)
        transaction = Transaction('P-', 0 , amount , int(self.__balance))
        self._Account__transaction.append(transaction)
        merch = bank.search_account('CUR001')
        merch.merchant(amount)
        if hasattr(debit_card , 'credit') :
            if amount >= 1000 : 
                edc = bank.search_edc_machine('EDC001')
                amount -= (amount - edc.calculate_cashback(bank , debit_card , amount))
...
class CurrentAccount(Account) :
    ...
    def merchant(self, amount):
        self._Account__balance += amount
        transaction = Transaction('PP-', 0 , amount , int(self._Account__balance))
        self._Account__transaction.append(transaction)


    
class FixedAccount(Account) :
     def __init__(self, account_no, user, month , balance):
        super().__init__(account_no , user , balance)
        self.__interest = 0.025 
        self.__balance = balance
        self.__months = month
     ...
     def withdraw(self, bank, channel_no, amount):
        if amount < 0 : return 'Error'
            ...
            if self.balance < amount: return 'Error: No initial deposit'
            current_date = datetime.now()
            months_elapsed = (current_date.year - self.__deposit_date.year) * 12 + (current_date.month - self.__deposit_date.month)

            if months_elapsed >= self.__months: 
                interest_earned = math.floor(self.balance * self.__interest * (self.__months / 12))
                self.add_balance(interest_earned)
                self.add_transaction(Transaction("I-", 0, interest_earned, int(self.balance)))

            if months_elapsed < self.__months :
                reduced_interest = self.__interest / 2  
                interest_earned = math.floor(self.balance * reduced_interest * (months_elapsed / 12))
                self.add_balance(interest_earned)
                self.add_transaction(Transaction("I-", 0, interest_earned, int(self.balance)))
            
            self.add_balance(-amount)
            self.add_transaction(Transaction("W-", channel_no, amount, int(self.balance)))
            return 'Success'
...
class ShoppingDebitCard(DebitCard) :
    def __init__(self, card_no, account_no, pin):
        super().__init__(card_no, account_no, pin)
        self.__credit = 0.01
        self.__pay_minimum = 1000 

    @property
    def pay_minimum(self) : return self.__pay_minimum
...
class BankingTest(unittest.TestCase):
    ...
    def test_fixed_deposit_initial(self): # 7. ทดสอบการฝากเงินเริ่มต้นในบัญชีเงินฝากประจำ
        ...
        fixed_account = FixedAccount("FIX002", self.tony, 12 , 100000.00)  # 12 months period
        initial_balance = fixed_account.balance
        
        # Perform initial deposit
        deposit_amount = 100000
        result = fixed_account.deposit(  self.bank ,"COUNTER:001", deposit_amount)
    ...
    def test_fixed_withdraw_before_maturity(self): # 8. ทดสอบการถอนเงินก่อนวันครบกำหนด 
        ...
        fixed_account = FixedAccount("FIX003", self.tony, 12 , 100000.00)  # 12 months period
        fixed_account.deposit( self.bank ,"COUNTER:001", initial_deposit)
        ...
        result = fixed_account.withdraw( self.bank ,"COUNTER:001", withdraw_amount)
    ...
    def test_fixed_withdraw_without_deposit(self): # 9. ทดสอบการถอนเงินโดยไม่มีการฝากเงินเริ่มต้น
        ...
        fixed_account = FixedAccount("FIX004", self.tony, 12, 0)
    ...
    def test_fixed_multiple_deposits(self): # 10. ทดสอบการฝากเงินหลายครั้งในบัญชีเงินฝาก
        ...
        # Create new fixed account
        fixed_account = FixedAccount("FIX005", self.tony, 12 , 0)
        
        # First deposit
        first_deposit = 100000
        result1 = fixed_account.deposit( self.bank , "COUNTER:001", first_deposit)
        self.assertEqual(result1, "Success", "First deposit should be successful")
        
        # Try second deposit
        second_deposit = 50000
        result2 = fixed_account.deposit(self.bank ,"COUNTER:001", second_deposit)
    ...
    def test_fixed_withdraw_at_maturity(self): # 11. ทดสอบการถอนเงินในวันครบกำหนด
        ...
        # Initial deposit
        initial_deposit = 100000
        fixed_account = FixedAccount("FIX006", self.tony, 12 , 0)  # 12 months period
        fixed_account.deposit( self.bank ,"COUNTER:001", initial_deposit)
    ...
    def test_current_account_merchant_payment(self): # 15. ทดสอบการชำระเงินผ่านบัญชีกระแสรายวันผ่าน EDC
        ...
        # Then make payment
        payment_result = edc.pay( self.bank ,self.steve_shopping_card, payment_amount)
        ...
        expected_customer_balance = customer_initial - payment_amount + edc.calculate_cashback(self.bank ,self.steve_shopping_card, payment_amount)
    ...
    def test_debit_card_annual_fee(self): # 16. ทดสอบการหักค่าธรรมเนียมประจำปีสำหรับบัตร debit
        ...
        def deduct_annual_fee(card, account):
            ...
            if isinstance(card, Card):
                result = account.withdraw(self.bank ,"SYSTEM", annual_fee)
                return result
    ...
    def test_thor_account_merchant_payment(self): # 18. ทดสอบการชำระเงินผ่านบัญชีของ Thor ผ่าน EDC (ไม่มีเงินคืน)
        ...
        # Then make payment
        payment_result = edc.pay( self.bank , self.thor_travel_card, payment_amount)
    ...
    def test_tony_atm_card_merchant_payment(self): # 19. ทดสอบการชำระเงินผ่านบัตร ATM ของ Tony ผ่าน EDC
        ...
        payment_result = edc.pay( self.bank , self.tony_atm_card, payment_amount)
